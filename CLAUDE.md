# guardian-crosswords — Claude Working Instructions

This document has two parts.

**Part 1** is specific to this repository: what it is, where things live, which streams
work is divided into, and what still needs setting up. **Part 2** is the shared
collaboration protocol, carried verbatim by every repo that uses it and delimited by
`BEGIN/END SHARED PROTOCOL` markers. Do not edit Part 2 to suit this repo — if it needs
to change, it needs to change everywhere. Repo-specific facts belong in Part 1, and Part 2
points at them rather than naming them.
...
A scraper that pulls Guardian crossword puzzles and serves them as a real-time multiplayer
web app: several players open the same puzzle and solve it together, with cell edits synced
live over WebSockets.

The distinction a newcomer most often gets wrong: there are **two** crossword-scraping
code paths in this repo's history. The root-level `README.md` describes the original
standalone script; the code that actually runs is `website/guardian_crosswords.py`, used by
the Flask backend to serve puzzle data to the app. Nothing at the repo root executes
anymore — everything live is under `website/`.

The other thing worth knowing: puzzle/game state (`games`, `room_states`, `game_players`,
`cell_owners` in `flask_app.py`) is **in-memory only** and lost on server restart. There is
no database. Treat anything touching game state as ephemeral by design, not as a bug to fix,
...
| Path | What it is |
|---|---|
| `website/flask_app.py` | Flask + Socket.IO backend: REST API, WebSocket real-time sync, serves the built SPA. **hub** — every new API route and socket event registers here. |
| `website/guardian_crosswords.py` | Scraper/parser: fetches Guardian puzzle data, builds `Crossword` objects, writes `.puz` files. **hub** — the `Crossword` class and `get_crossword_data()` are the data contract the backend depends on. |
| `website/frontend/src/App.jsx` | Main React app: grid rendering, input handling, completion detection, multiplayer wiring. **hub** — almost every frontend feature touches this file. |
| `website/frontend/src/PlayerOverlay.jsx` | Multiplayer player list/cursor overlay UI. |
| `website/frontend/src/AnagramHelper.jsx` | Anagram-solving helper tool UI. |
| `website/frontend/package.json` | Frontend dependencies and build scripts. **hub** |
| `website/requirements.txt` | Backend Python dependencies. **hub** |
| `website/templates/index.html` | Legacy Flask-rendered template, pre-dates the React SPA. |
| `render.yaml` | Render.com deploy config (build + start commands). |
| `outputs/`, `website/output_files/` | Generated `.puz` files and scrape output. Gitignored — not source, never migrate content from here. |

## Streams

| Stream | Issue prefix | Concern |
|---|---|---|
| Frontend | `W*-Frontend` | `website/frontend/**` — grid UI, multiplayer client state, rendering |
| Backend | `W*-Backend` | `website/flask_app.py`, `website/guardian_crosswords.py`, `website/requirements.txt` — API, sockets, scraping, deploy |

Two streams for now, cut along the frontend/backend hub-file boundary — the two hub files
that matter (`App.jsx` vs. `flask_app.py`/`guardian_crosswords.py`) are essentially never
touched by the same fix. Revisit this if a third concern (e.g. the scraper) grows enough
distinct work to justify its own stream and its own hub-file boundary.

## Roadmap

No separate roadmap document yet — small enough to keep here.

**Wave 1 completion criterion:** the four currently-known UX bugs are fixed and deployed
(mobile rendering, completion-message copy, completion-detection after reveal, an undo
control), and nothing else regresses. There is no wave 2 defined yet; open a new milestone
and describe it here when one is needed.

Both streams are live in wave 1. `W1-Backend` currently has no filed issues — it exists so
backend/scraper work has somewhere to land without a milestone-creation detour.

## Repo-specific bindings

| Setting | Value |
|---|---|
| **Worktree root** | `.claude/worktrees/` (gitignored) |
| **Roadmap document** | This file, § Roadmap |
| **Decision archive** | none — no pre-migration `DECISIONS.md`/`TODO.md` existed |
...
- [x] Create the labels
- [x] Write the roadmap (§ Roadmap above)
- [x] Create the milestones (`W1-Frontend`, `W1-Backend`)
- [x] Migrate open tasks to issues — the 4 pre-existing open issues were already issues;
      they've been labelled `task`, milestoned, and given predicted paths/hub files
- [x] Freeze existing archives — none existed, nothing to freeze
- [x] Gitignore the worktree root

Migration is complete. Part 2's task and decision mechanics are fully in effect — nothing
